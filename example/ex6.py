...
class Neural_Network():
    def __init__(self, inputSize=2, hiddensize=1, n_ensemble=16):
        ...
        # weights
        self.W1 = np.random.randn(self.n_ensemble, self.inputSize, self.hiddensize)
        self.b1 = np.random.randn(self.n_ensemble, 1, self.hiddensize)
        self.W2 = np.random.randn(self.n_ensemble, self.hiddensize, self.outputSize)

    def forward(self, X):
        X = np.tile(X, (self.n_ensemble, 1, 1))
        self.z1 = np.matmul(X, self.W1) + self.b1
        self.h1 = self.tanh(self.z1)
        dzdt = np.matmul(self.h1, self.W2)   # + self.b2
        print('dzdt shape:', dzdt.shape)  # (16, 64, 2)
        dzdt = np.sum(dzdt, axis=0)
        print('dzdt shape:', dzdt.shape)  # (64, 2)


        dhdz = self.h1 * np.matmul(self.W2, self.W1)
    ...
    def grad(self, a, X):
        h1_grad = self.tanhPrime(self.h1)
        tmp = []
